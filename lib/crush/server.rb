...
# Crush::Connection::Local call.  The results are sent back across the wire to
# be decoded by Crush::Connection::Remote on the other side.
class CrushHandler < Mongrel::HttpHandler
	...
	def log(msg)
		File.open('crushd.log', 'a') do |f|
			f.puts "#{Time.now.strftime('%Y-%m-%d %H:%I:%S')} :: #{msg}"
		end
	end
end

# A container class to run the Mongrel server for crushd.
class CrushServer
	def run
		host = "127.0.0.1"
		port = Crush::Config::DefaultPort

		crushd = CrushHandler.new
		crushd.log "crushd listening on #{host}:#{port}"
