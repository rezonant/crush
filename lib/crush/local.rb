...
# Crush::Box uses a connection object to execute all rush commands.  If the box
# is local, Crush::Connection::Local is created.  The local connection is the
...
# modifying files, getting process lists, and so on.  CrushServer creates a
...
# Crush::Connection::Local#receive.
class Crush::Connection::Local
	...
	def rename(path, name, new_name)
		raise(Crush::NameCannotContainSlash, "#{path} rename #{name} to #{new_name}") if new_name.match(/\//)
		old_full_path = "#{path}/#{name}"
		new_full_path = "#{path}/#{new_name}"
		raise(Crush::NameAlreadyExists, "#{path} rename #{name} to #{new_name}") if ::File.exists?(new_full_path)
	...
	def copy(src, dst)
		...
		raise Crush::DoesNotExist, File.dirname(dst)
	rescue RuntimeError
		raise Crush::DoesNotExist, src
	end
	...
	def read_archive(full_path)
		`cd #{Crush.quote(::File.dirname(full_path))}; tar c #{Crush.quote(::File.basename(full_path))}`
	end

	# Extract an in-memory archive to a dir.
	def write_archive(archive, dir)
		IO.popen("cd #{Crush::quote(dir)}; tar x", "w") do |p|
			p.write archive
		end
	end
	...
	def index(base_path, glob)
		...
		::Dir.chdir(base_path) do
			::Dir.glob(glob, ::File::FNM_DOTMATCH).each do |fname|
				next if ['..', '.'].member? fname
		...
		raise Crush::DoesNotExist, base_path
	end
	...
	def size(full_path)
		`du -sb #{Crush.quote(full_path)}`.match(/(\d+)/)[1].to_i
	end
	...
	# for use in the Crush::Process#new.
	...
	# suitable for use in the Crush::Process#new.
	def parse_ps(line)
		m = line.split(" ", 6)
		params = {}
		params[:pid] = m[0]
	...
	def bash(command, user=nil, background=false, reset_environment=false)
		...
		raise(Crush::BashFailed, err) if retval != 0
	...
	# Raised when the action passed in by CrushServer is not known.
	class UnknownAction < Exception; end

	# CrushServer uses this method to transform a hash (:action plus parameters
	...
	def receive(params)
		case params[:action]
			...
			when 'set_access'     then set_access(params[:full_path], Crush::Access.from_hash(params))
