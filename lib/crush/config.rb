...
# The config class accesses files in ~/.crush to load and save user preferences.
class Crush::Config
	DefaultPort = 7770

	attr_reader :dir

	# By default, reads from the dir ~/.crush, but an optional parameter allows
	# using another location.
	def initialize(location=nil)
		@dir = Crush::Dir.new(location || "#{ENV['HOME']}/#{Crush::ConfigDir}")
		@dir.create
	end
	...
	# or .profile.
	#
	# Example ~/.crush/env.rb:
	#
	#   server = Crush::Box.new('www@my.server')
	...
	# Commands are mixed in to Array and Crush::Entry, alongside the default
	# commands from Crush::Commands.  Any methods here should reference "entries"
	# to get the list of entries to operate on.
	#
	# Example ~/.crush/commands.rb:
	#
	...
	# remote access via crushd.  You can fill this in manually, or let the remote
	...
	# ~/.crush/tunnels contains a list of previously created ssh tunnels.  The
