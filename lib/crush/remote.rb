...
# This class it the mirror of Crush::Connection::Local.  A Crush::Box which is
...
class Crush::Connection::Remote
	attr_reader :host, :tunnel

	def initialize(host)
		@host = host
		@tunnel = Crush::SshTunnel.new(host)
	end
	...
	# object), send it across the wire to the CrushServer listening on the other
	# side.  Uses http basic auth, with credentials fetched from the Crush::Config.
	def transmit(params)
		ensure_tunnel
	...
	def process_result(code, body)
		raise Crush::NotAuthorized if code == "401"
		...
		raise Crush::FailedTransmit if code != "200"
	...
	def parse_exception(body)
		...
		raise "invalid exception class: #{klass}" unless klass.match(/^Crush::[A-Za-z]+$/)
