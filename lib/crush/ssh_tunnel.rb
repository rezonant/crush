...
# HTTP commands can be sent by Crush::Connection::Remote.
class Crush::SshTunnel
	...
	def ssh_append_to_credentials(string)
		...
		config_dir = Crush.ConfigDir
		passwords_file = "~/#{config_dir}/passwords"
		string = "'#{string}'"
		ssh "M=`grep #{string} #{passwords_file} 2>/dev/null | wc -l`; " +
			"if [ $M = 0 ]; " +
				"then mkdir -p #{config_dir}; " +
				"chmod 700 #{config_dir}; " +
				"echo #{string} >> #{passwords_file}; " +
	...
	def tunnel_options
		{
			:local_port => @port,
			:remote_port => Crush::Config::DefaultPort,
			:ssh_host => @real_host,
		}
	end
	...
	def next_available_port
		(config.tunnels.values.max || Crush::Config::DefaultPort) + 1
	end
