...
describe Crush::SshTunnel do
	before do
		@tunnel = Crush::SshTunnel.new('spec.example.com')
	...
	it "ensure_tunnel sets everything up for the tunnel when one does not already exist" do
		...
		@tunnel.should_receive(:launch_crushd)
	...
	it "establishes a tunnel and saves it to ~/.crush/tunnels" do
	...
	it "ssh_tunnel_command request that the port be set" do
		...
		lambda { @tunnel.ssh_tunnel_command }.should raise_error(Crush::SshTunnel::NoPortSelectedYet)
	end
	...
	it "launches crushd on the remote host via ssh" do
		@tunnel.should_receive(:ssh) do |cmd|
			cmd.should match(/crushd/)
		end
		@tunnel.launch_crushd
	end
