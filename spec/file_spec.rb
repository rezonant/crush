...
describe Crush::File do
	...
	it "is a child of Crush::Entry" do
		@file.should be_kind_of(Crush::Entry)
	end
	...
	it "can create itself as a blank file, and return itself" do
		create_me = Crush::File.new("#{@sandbox_dir}/create_me")
	...
	it "can fetch contents or blank if doesn't exist" do
		Crush::File.new('/does/not/exist').contents_or_blank.should == ""
	end

	it "can fetch lines, or empty if doesn't exist" do
		Crush::File.new('/does/not/exist').lines_or_empty.should == []
	end
end
