...
describe Crush::Connection::Local do
	before do
		...
		@con = Crush::Connection::Remote.new('spec.example.com')
	end
	...
	it "an http result code of 401 raises NotAuthorized" do
		lambda { @con.process_result("401", "") }.should raise_error(Crush::NotAuthorized)
	end

	it "an http result code of 400 raises the exception passed in the result body" do
		@con.stub!(:parse_exception).and_return(Crush::DoesNotExist, "message")
		lambda { @con.process_result("400", "") }.should raise_error(Crush::DoesNotExist)
	end

	it "an http result code of 501 (or anything other than the other defined codes) raises FailedTransmit" do
		lambda { @con.process_result("501", "") }.should raise_error(Crush::FailedTransmit)
	end

	it "parse_exception takes the class from the first line and the message from the second" do
		@con.parse_exception("Crush::DoesNotExist\nthe message\n").should == [ Crush::DoesNotExist, "the message" ]
	end

	it "parse_exception rejects unrecognized exceptions" do
		lambda { @con.parse_exception("NotACrushException\n") }.should raise_error
	end
