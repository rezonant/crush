...
describe Crush::Connection::Local do
	before do
		@sandbox_dir = "/tmp/crush_spec.#{Process.pid}"
		system "rm -rf #{@sandbox_dir}; mkdir -p #{@sandbox_dir}"

		@con = Crush::Connection::Local.new
	end
	...
	it "receive -> set_access(full_path, user, group, permissions)" do
		access = mock("access")
		Crush::Access.should_receive(:from_hash).with(:action => 'set_access', :full_path => 'full_path', :user => 'joe').and_return(access)
	...
	it "receive -> unknown action exception" do
		lambda { @con.receive(:action => 'does_not_exist') }.should raise_error(Crush::Connection::Local::UnknownAction)
	end
	...
	it "file_contents raises DoesNotExist if the file does not exist" do
		...
		lambda { @con.file_contents(fname) }.should raise_error(Crush::DoesNotExist, fname)
	end
	...
	it "copy raises DoesNotExist with source path if it doesn't exist or otherwise can't be accessed" do
		lambda { @con.copy('/does/not/exist', '/tmp') }.should raise_error(Crush::DoesNotExist, '/does/not/exist')
	end

	it "copy raises DoesNotExist with destination path if it can't access the destination" do
		lambda { @con.copy('/tmp', '/does/not/exist') }.should raise_error(Crush::DoesNotExist, '/does/not')
	end
	...
	it "index raises DoesNotExist when the base path is invalid" do
		lambda { @con.index('/does/not/exist', '*') }.should raise_error(Crush::DoesNotExist, '/does/not/exist')
	end
	...
	it "stat raises DoesNotExist if the entry does not exist" do
		fname = "#{@sandbox_dir}/does_not_exist"
		lambda { @con.stat(fname) }.should raise_error(Crush::DoesNotExist, fname)
	end
...
EOPS
	...
	it "executes a bash command, raising and error (with stderr as the message) when return value is nonzero" do
		lambda { @con.bash("no_such_bin") }.should raise_error(Crush::BashFailed, /command not found/)
	end
